...
- move upward and downward through transcript review one rendered line at a time instead of jumping to the top of a larger review buffer
...
- a stepped review mode where each glasses `Up` or `Down` gesture shifts the wrapped transcript window by one rendered line
...
The current follow-up regression proof closes the reported `Up` jump bug too: one glasses `Up` now shifts the wrapped transcript by one rendered line instead of snapping to the top of the review buffer.
